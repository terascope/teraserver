...
module.exports = function(appConfig, date_field) {
    ...
    function performSearch(context, req, res, config) {
        ...
        if (config.geo_field) {
            ...
            if (geo_search) {
                context.body.sort =  [geo_search.sort];
    ...
    function geoSearch(req, res, geo_field) {
        ...
        if (query.geo_box_top_left || query.geo_point || query.geo_distance || query.geo_sort_point) {
            ...
            if (query.geo_box_top_left) {
                ...
                if (isGeoSort) {
                    var sortedSearch = {"geo_bounding_box": {}};
                    sortedSearch.geo_bounding_box[geo_field] = {
                        "lat": geoSortPoint[0],
                        "lon": geoSortPoint[1]
                    };
                    sortedSearch.geo_bounding_box.order = geoSortOrder;
                    sortedSearch.geo_bounding_box.unit = geoSortUnit;

                    queryResults.sort = sortedSearch;
                }
            ...
            if (query.geo_distance) {
                ...
                sortedSearch._geo_distance[geo_field] = {
                    "lat": locationPoints[0],
                    "lon": locationPoints[1]
                };
