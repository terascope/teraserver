    ...
    function performSearch(context, req, res, config) {
        if (req.query.size) {
            ...
            if (typeof size === "object") {
                res.status(400).json({error: "size parameter must be a number, was given " + JSON.stringify(size)});
                return;
            }

            if (isNaN(Number.parseInt(size))) {
                res.status(400).json({error: "size parameter must be a valid number, was given " + size});
                return;
            }
        }
        ...
        if (req.query.start) {
            if (isNaN(req.query.start)) {
                res.status(400).json({error: "the start parameter must be a number, was given: " + req.query.start});
                return;
            }
            else {
        ...
        if (config.sort_enabled && req.query.sort) {
            ...
            if (config.sort_dates_only && pieces[0].toLowerCase() !== date_field) {
                res.status(400).json({error: "Invalid sort parameter. Sorting currently available for the '" + date_field + "' field only."});
                return;
            }

            if (pieces.length != 2 || (pieces[1].toLowerCase() != 'asc' && pieces[1].toLowerCase() != 'desc')) {
                res.status(400).json({error: "Invalid sort parameter. Must be field_name:asc or field_name:desc."});
                return;
            }
        ...
        if (req.query.fields || config.allowed_fields) {
            ...
            if (req.query.fields) {
                ...
                if (finalFields.length === 0) {
                    res.status(400).json({error: 'the fields parameter does not contain any valid fields'});
                    return;
                }
    ...
    function luceneWithHistoryQuery(req, res, indexes, config) {
        ...
        if (req.query && req.query.history) {
            ...
            if (Number.isNaN(history) || Number.isNaN(start)) {
                res.status(400).json({error: "History specification must be numeric."});
                return;
            }

            if (history < 0 || start < 0) {
                res.status(400).json({error: "History specification must be a positive number."});
                return;
            }

            if ((history + start) > 90) {
                res.status(400).json({error: "History is not available beyond 90 days."});
                return;
            }
    ...
    function luceneQuery(req, res, index, config) {
        if (!req.query || !req.query.q) {
            if (config.require_query !== false) {
                res.status(400).json({error: "Search query must be specified in the query parameter q."});
                return;
            }
        }
        ...
        if (re.test(lucQuery)) {
            if (!properQuery(lucQuery, re)) {
                res.status(400).json({error: "Wild card queries of the form 'fieldname:*value' or 'fieldname:?value' can not be evaluated. Please refer to the documentation on 'fieldname.right'."});
                return;
            }
        }
    ...
    function geoSearch(req, res, geo_field) {
        ...
        if (query.geo_box_top_left || query.geo_point || query.geo_distance) {
            if (query.geo_box_top_left && query.geo_point) {
                res.status(400).json({error: "geo_box and geo_distance queries can not be combined."});
                return;
            }

            if ((query.geo_point && !query.geo_distance) || (!query.geo_point && query.geo_distance)) {
                console.log('i should be sending back error');
                res.status(400).json({error: "values for geo_point or geo_distance in the query are missing, they must be used in conjunction, please verify your query"});
                return;
            }
            ...
            if (query.geo_box_top_left) {
                ...
                if (top_left.length != 2) {
                    res.status(400).json({error: "Invalid geo_box_top_left"});
                    return;
                }

                var bottom_right = geo_point(query.geo_box_bottom_right);
                if (bottom_right.length != 2) {
                    res.status(400).json({error: "Invalid geo_box_bottom_right"});
                    return;
                }
            ...
            if (query.geo_distance) {
                if (!valid_geo_distance(query.geo_distance)) {
                    res.status(400).json({error: "geo_distance must be in units of 'mi', 'yd', 'ft', 'km' or 'm'"});
                    return;
                }

                var location = geo_point(query.geo_point);
                if (location.length != 2) {
                    res.status(400).json({error: "Invalid geo_point"});
                    return;
                }
