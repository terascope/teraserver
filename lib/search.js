...
module.exports = function (_appConfig, _dateField) {
    ...
    function performSearch(context, req, res, config) {
        try {
            ...
            return searchResult.then((response) => {
                ...
                if (response.hits) {
                    let results = [];
                    for (let i = 0; i < response.hits.hits.length; i += 1) {
                ...
                .finally(() => {
                    const end = Date.now();
                    searchCount.tokenAndStatusCodeCount(req, res, end - startTime);
                });
        } catch (err) {
            ...
            searchCount.tokenAndStatusCodeCount(req, res, null);
        }
    }
