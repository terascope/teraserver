...
module.exports = function(appConfig, date_field) {
    ...
    function performSearch(context, req, res, config) {
        ...
        if (req.query.size) {
            ...
            if (typeof size === "object") {
                sendError(res, 500, req.query, debug, `size parameter must be a number, was given ${JSON.stringify(size)}`);
                return;
            }

            if (isNaN(Number.parseInt(size))) {
                sendError(res, 500, req.query, debug, `size parameter must be a valid number, was given ${size}`);
                return;
            }
        }
        ...
        if (req.query.start) {
            if (isNaN(req.query.start)) {
                sendError(res, 500, req.query, debug, `the start parameter must be a number, was given: ${req.query.start}`);
                return;
            }
            else {
        ...
        if (req.query.type) {
            ...
            if (typeof req.query.type !== 'string') {
                sendError(res, 500, req.query, debug, `the type parameter must be a string was given: ${typeof req.query.type}`);
                return;
            }
            else {
                bool_clause.push({
        ...
        if (config.sort_enabled && req.query.sort) {
            ...
            if (typeof req.query.sort !== "string") {
                sendError(res, 500, req.query, debug, `the sort parameter must be a string was given: ${typeof req.query.sort}`);
                return;
            }
            ...
            if (pieces.length != 2 || (pieces[1].toLowerCase() != 'asc' && pieces[1].toLowerCase() != 'desc')) {
                sendError(res, 500, req.query, debug, `Invalid sort parameter. Must be field_name:asc or field_name:desc`);
                return;
            }
        ...
        if (req.query.fields || config.allowed_fields) {
            ...
            if (req.query.fields && (typeof req.query.fields !== "string")) {
                sendError(res, 500, req.query, debug, `the fields parameter must be a string was given: ${typeof req.query.fields}`);
                return;
            }
            var finalFields;
            ...
            if (req.query.fields) {
                ...
                if (finalFields.length === 0) {
                    sendError(res, 500, req.query, debug, `the fields parameter does not contain any valid fields`);
                    return;
                }
    ...
    function luceneWithHistoryQuery(req, res, indexes, config) {
        ...
        if (req.query && req.query.history) {
            ...
            if (Number.isNaN(history) || Number.isNaN(start)) {
                sendError(res, 500, req.query, debug, `History specification must be numeric`);
                return;
            }

            if (history < 0 || start < 0) {
                sendError(res, 500, req.query, debug, `History specification must be a positive number`);
                return;
            }

            if ((history + start) > 90) {
                sendError(res, 500, req.query, debug, `History is not available beyond 90 days`);
                return;
            }
    ...
    function luceneQuery(req, res, index, config) {
        ...
        if (!req.query || !req.query.q) {
            if (config.require_query !== false) {
                sendError(res, 500, req.query, debug, `Search query must be specified in the query parameter q`);
                return;
            }
        }
        ...
        if (config.require_query === true && req.query.q) {
            if (typeof req.query.q !== "string") {
                sendError(res, 500, req.query, debug, `the q parameter must be a string was given: ${typeof req.query.q}`);
                return;
            }
        }
        ...
        if (re.test(lucQuery)) {
            if (!properQuery(lucQuery, re)) {
                sendError(res, 500, req.query, debug, `Wild card queries of the form 'fieldname:*value' or 'fieldname:?value' can not be evaluated. Please refer to the documentation on 'fieldname.right'`);
                return;
            }
        }
    ...
    function geoSearch(req, res, geo_field, config) {
        ...
        if (query.geo_box_top_left || query.geo_point || query.geo_distance) {
            if (query.geo_box_top_left && query.geo_point) {
                sendError(res, 500, req.query, debug, `geo_box and geo_distance queries can not be combined`);
                return;
            }

            if ((query.geo_point && !query.geo_distance) || (!query.geo_point && query.geo_distance)) {
                sendError(res, 500, req.query, debug, `values for geo_point or geo_distance in the query are missing, they must be used in conjunction, please verify your query`);
                return;
            }
            ...
            if (query.geo_box_top_left) {
                ...
                if (top_left.length != 2) {
                    sendError(res, 500, req.query, debug, `Invalid geo_box_top_left`);
                    return;
                }

                var bottom_right = geo_point(query.geo_box_bottom_right);
                if (bottom_right.length != 2) {
                    sendError(res, 500, req.query, debug, `Invalid geo_box_bottom_right`);
                    return;
                }
            ...
            if (query.geo_distance) {
                if (!valid_geo_distance(query.geo_distance)) {
                    sendError(res, 500, req.query, debug, `geo_distance must be in units of 'mi', 'yd', 'ft', 'km' or 'm'`);
                    return;
                }

                var location = geo_point(query.geo_point);
                if (location.length != 2) {
                    sendError(res, 500, req.query, debug, `Invalid geo_point`);
                    return;
                }
    ...
    function validateDateRange(res, date_start, date_end) {
        ...
        if (message) {
            sendError(res, 500, req.query, debug, message);
            return false;
        }
        else {
            return true;
