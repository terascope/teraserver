...
const stats = require('./api_analytics');
...
module.exports = function(appConfig, date_field) {
    ...
    function performSearch(context, req, res, config) {
        ...
        client.search(context, function(err, response) {
    ...
    function luceneQuery(req, res, index, config) {
        ...
        if (re.test(lucQuery)) {
            if (!properQuery(lucQuery, re)) {
                ...
                stats.tokenAndStatusCodeCount(req, res, undefined);
                return;
            }
        }
        ...
        try {
            ...
            const end = Date.now();
            stats.tokenAndStatusCodeCount(req, res, end - start);
        }
        catch (err) {
