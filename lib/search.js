...
module.exports = function(appConfig, date_field) {
    ...
    function performSearch(context, req, res, config) {
        ...
        if (req.query.size) {
            ...
            if (typeof size === "object") {
                sendError(res, 500, req.query, debug, null, `size parameter must be a number, was given ${JSON.stringify(size)}`, isPretty);
                return;
            }

            if (isNaN(Number.parseInt(size))) {
                sendError(res, 500, req.query, debug, null, `size parameter must be a valid number, was given ${size}`, isPretty);
                return;
            }
        }
        ...
        if (req.query.size && req.query.size > 100000) {
            sendError(res, 500, req.query, debug, null, `Request size too large. Must be less than ${max_query_size}.`, isPretty);
            return;
        }
        ...
        if (config.date_range) {
            if (!validateDateRange(res, req, isPretty, debug)) {
                return;
            }
        ...
        if (req.query.start) {
            if (isNaN(req.query.start)) {
                sendError(res, 500, req.query, debug, null, `the start parameter must be a number, was given: ${req.query.start}`, isPretty);
                return;
            }
            else {
        ...
        if (req.query.type) {
            ...
            if (typeof req.query.type !== 'string') {
                sendError(res, 500, req.query, debug, null, `the type parameter must be a string was given: ${typeof req.query.type}`, isPretty);
                return;
            }
            else {
                bool_clause.push({
        ...
        if (config.sort_enabled && req.query.sort) {
            ...
            if (typeof req.query.sort !== "string") {
                sendError(res, 500, req.query, debug, null,`the sort parameter must be a string was given: ${typeof req.query.sort}`, isPretty);
                return;
            }

            var pieces = req.query.sort.split(':');
            if (config.sort_dates_only && pieces[0].toLowerCase() !== date_field) {
                sendError(res, 500, req.query, debug, null, `Invalid sort parameter. Sorting currently available for the '${date_field}' field only`);
                return;
            }

            if (pieces.length != 2 || (pieces[1].toLowerCase() != 'asc' && pieces[1].toLowerCase() != 'desc')) {
                sendError(res, 500, req.query, debug, null, `Invalid sort parameter. Must be field_name:asc or field_name:desc`, isPretty);
                return;
            }
        ...
        if (req.query.fields || config.allowed_fields) {
            ...
            if (req.query.fields) {
                ...
                if (finalFields.length === 0) {
                    sendError(res, 500, req.query, debug, null, `the fields parameter does not contain any valid fields`, isPretty);
                    return;
                }
        ...
        client.search(context, function(_error, response) {
            ...
            if (response.hits) {
                ...
                sendError(res, 500, req.query, debug, null, `No results returned from query`, isPretty);
            }
        });
    }
    ...
    function luceneWithHistoryQuery(req, res, indexes, config) {
        ...
        if (req.query && req.query.history) {
            ...
            if (Number.isNaN(history) || Number.isNaN(start)) {
                sendError(res, 500, req.query, debug, null, `History specification must be numeric`, isPretty);
                return;
            }

            if (history < 0 || start < 0) {
                sendError(res, 500, req.query, debug, null, `History specification must be a positive number`, isPretty);
                return;
            }

            if ((history + start) > 90) {
                sendError(res, 500, req.query, debug, null, `History is not available beyond 90 days`, isPretty);
                return;
            }
    ...
    function luceneQuery(req, res, index, config) {
        ...
        if (!req.query || !req.query.q) {
            if (config.require_query !== false) {
                sendError(res, 500, req.query, debug, null, `Search query must be specified in the query parameter q`, isPretty);
                return;
            }
        }
        ...
        if (config.require_query === true && req.query.q) {
            if (typeof req.query.q !== "string") {
                sendError(res, 500, req.query, debug, null, `the q parameter must be a string was given: ${typeof req.query.q}`, isPretty);
                return;
            }
        }
        ...
        if (re.test(lucQuery)) {
            if (!properQuery(lucQuery, re)) {
                sendError(res, 500, req.query, debug, null, `Wild card queries of the form 'fieldname:*value' or 'fieldname:?value' can not be evaluated. Please refer to the documentation on 'fieldname.right'`, isPretty);
                return;
            }
        }
        ...
        if (config.allowed_fields) {
            ...
            if (failures.length >= 1) {
                sendError(res, 400, req.query, debug, null, `you cannot query on these terms: ${failures.join('')}`, isPretty);
                return
            }
        }
    ...
    function geoSearch(req, res, geo_field, config) {
        ...
        if (query.geo_box_top_left || query.geo_point || query.geo_distance) {
            if (query.geo_box_top_left && query.geo_point) {
                sendError(res, 500, req.query, debug, null, `geo_box and geo_distance queries can not be combined`, isPretty);
                return;
            }

            if ((query.geo_point && !query.geo_distance) || (!query.geo_point && query.geo_distance)) {
                sendError(res, 500, req.query, debug, null, `values for geo_point or geo_distance in the query are missing, they must be used in conjunction, please verify your query`, isPretty);
                return;
            }

            if ((query.geo_point && !query.geo_distance) || (!query.geo_point && query.geo_distance)) {
                sendError(res, 500, req.query, debug, null, `Both geo_point and geo_distance must be provided for a geo_point query`, isPretty);
                return;
            }
            ...
            if (query.geo_box_top_left) {
                ...
                if (top_left.length != 2) {
                    sendError(res, 500, req.query, debug, null, `Invalid geo_box_top_left`, isPretty);
                    return;
                }

                var bottom_right = geo_point(query.geo_box_bottom_right);
                if (bottom_right.length != 2) {
                    sendError(res, 500, req.query, debug, null, `Invalid geo_box_bottom_right`, isPretty);
                    return;
                }
            ...
            if (query.geo_distance) {
                if (!valid_geo_distance(query.geo_distance)) {
                    sendError(res, 500, req.query, debug, null, `geo_distance must be in units of 'mi', 'yd', 'ft', 'km' or 'm'`, isPretty);
                    return;
                }

                var location = geo_point(query.geo_point);
                if (location.length != 2) {
                    sendError(res, 500, req.query, debug, null, `Invalid geo_point`, isPretty);
                    return;
                }
    ...
    function validateDateRange(res, req, isPretty, debug) {
        var message = "";
        ...
        if (message) {
            sendError(res, 500, req.query, debug, null, message, isPretty);
            return false;
        }
        else {
            return true;
