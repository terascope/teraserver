...
module.exports = function (context, app, express) {
    ...
    var api = {
        ...
        load: function () {
            ...
            if (config.TeraServer.plugins && config.TeraServer.plugins.length > 0) {
                for (var i = 0; i < config.TeraServer.plugins.length; i++) {
                    var plugin = loadPlugin(config.TeraServer.plugins[i]);
    ...
    function loadPlugin(name) {
        ...
        if (name === 'teranaut') {
            plugin = require(__dirname + '/../plugins/teranaut')
        }
        else {
            plugin = require(name);
        }
