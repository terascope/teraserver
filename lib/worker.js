...
module.exports = (context) => {
    ...
    function setupServer() {
        if (fs.existsSync(`${config.teraserver.ssl_path}/key.pem`)) {
            ...
            server.setTimeout(config.teraserver.request_timeout);
    ...
    function errorHandler(err, req, res, next) {
        ...
        const statusCodeMsg = statusCode ? `statusCode: ${statusCode}` : '';
        const reason = `Unexpected error occurred. ${codeMsg}${statusCodeMsg}`;

        if (!errMsg) {
            logger.error(req, reason);
        } else if (errMsg.length > 1000) {
            logger.error(errMsg, reason);
        }
